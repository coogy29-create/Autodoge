		...
		then
			...
			if (not knownOnly) or profile.Stable then
				...
				local reverseUncertainty =
					os.clock() < (data.ReverseUntil or 0)
...
local function interpolateThreatPosition(threat, t)
	...
	for i = 2, #THREAT_KNOTS do
		...
		if t <= rightTime then
			...
			return threat.Positions[i - 1]:Lerp(
				threat.Positions[i],
				alpha
			)
		end
	...
	return threat.Positions[#threat.Positions]
end
...
local function chooseDirection(character, humanoid, root, knownOnly, learningKnownDodge)
	...
	-- Expensive future-exit analysis is limited to the best few first steps.
