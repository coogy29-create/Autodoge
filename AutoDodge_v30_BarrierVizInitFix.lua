...
local BARRIER_PROACTIVE_DISTANCE = 15.0
local BARRIER_PROACTIVE_LOSS_WEIGHT = 155
local BARRIER_PROACTIVE_ESCAPE_BONUS = 32
...
local SHOW_BARRIER_RANGES = false
local BARRIER_VIZ_OUTER_TRANSPARENCY = 0.93
local BARRIER_VIZ_COMFORT_TRANSPARENCY = 0.88
local BARRIER_VIZ_STRONG_TRANSPARENCY = 0.82
local BARRIER_VIZ_HARD_TRANSPARENCY = 0.72
...
local VizToggle = Instance.new("TextButton")
VizToggle.Size = UDim2.new(1, -20, 0, 39)
VizToggle.Position = UDim2.fromOffset(10, 188)
VizToggle.BackgroundColor3 = Color3.fromRGB(58, 58, 66)
VizToggle.BorderSizePixel = 0
VizToggle.Text = "Barrier 표시 OFF"
VizToggle.TextColor3 = Color3.fromRGB(255, 255, 255)
VizToggle.TextSize = 14
VizToggle.Font = Enum.Font.GothamBold
VizToggle.ZIndex = 12
VizToggle.Parent = Main

local VizCorner = Instance.new("UICorner")
VizCorner.CornerRadius = UDim.new(0, 10)
VizCorner.Parent = VizToggle
...
local BarrierVizFolder = AutoDodgeDataFolder:FindFirstChild("BarrierVisualization")
if not BarrierVizFolder then
	BarrierVizFolder = Instance.new("Folder")
	BarrierVizFolder.Name = "BarrierVisualization"
	BarrierVizFolder.Parent = AutoDodgeDataFolder
end

local function clearBarrierVisualization()
	for _, child in ipairs(BarrierVizFolder:GetChildren()) do
		child:Destroy()
	end
end

local function createBarrierVizPart(parent, name, barrierPart, expandDistance, transparency, color)
	...
	part.Size = Vector3.new(
		barrierPart.Size.X + expandDistance * 2,
		barrierPart.Size.Y + 0.15,
		barrierPart.Size.Z + expandDistance * 2
	)
	part.CFrame = barrierPart.CFrame
	part.Parent = parent
	return part
end

local function rebuildBarrierVisualization()
	clearBarrierVisualization()

	if not SHOW_BARRIER_RANGES then
		return
	end
	...
	for _, barrierPart in ipairs(BarrierParts) do
		if barrierPart and barrierPart.Parent then
			pcall(function()
				...
				model.Name = barrierPart.Name .. "_Viz"
				model.Parent = BarrierVizFolder

				createBarrierVizPart(
					model,
					"ProactiveRange",
					barrierPart,
					BARRIER_PROACTIVE_DISTANCE,
					BARRIER_VIZ_OUTER_TRANSPARENCY,
				...
				createBarrierVizPart(
					...
					BARRIER_VIZ_COMFORT_TRANSPARENCY,
				...
				createBarrierVizPart(
					...
					BARRIER_VIZ_STRONG_TRANSPARENCY,
					Color3.fromRGB(255, 135, 60)
				)
...
workspace.DescendantAdded:Connect(function(obj)
	if isBarrierPart(obj) then
		...
		if SHOW_BARRIER_RANGES then
			rebuildBarrierVisualization()
		end
	end
end)
...
local function evaluateSpacePath(root, humanoid, direction, threatCount)
	...
	-- Start discouraging motion toward a Barrier before we are already
	-- against it. Previously this only started inside 9 studs, which
	-- allowed repeated dodges to slowly walk the player into a corner.
	if nearestBarrier
		and currentBarrierDistance < BARRIER_PROACTIVE_DISTANCE
	then
		local towardDot =
			unit:Dot(nearestBarrier.RayDirection)

		local proactiveCloseness =
			1 - math.clamp(
		...
		if towardDot > 0 then
			...
			result.Risk +=
				...
				* (0.55 + proactiveCloseness * 3.5)
				* denseMultiplier
		else
			local awayAmount =
				math.clamp(-towardDot, 0, 1)
...
local function betterCandidate(a, b)
	...
	-- Never prefer a wall-safe move that actually gets hit.
	...
	local aNearWall =
		(a.Eval.SpaceFutureBarrierDistance or math.huge)
			< BARRIER_PROACTIVE_DISTANCE

	local bNearWall =
		(b.Eval.SpaceFutureBarrierDistance or math.huge)
			< BARRIER_PROACTIVE_DISTANCE

	if aNearWall or bNearWall then
		local aLoss =
		...
		if math.abs(aLoss - bLoss) > BARRIER_DISTANCE_EPSILON then
			return aLoss < bLoss
		end
		...
		if math.abs(aFuture - bFuture) > 0.35 then
...
VizToggle.Activated:Connect(function()
	SHOW_BARRIER_RANGES = not SHOW_BARRIER_RANGES

	if SHOW_BARRIER_RANGES then
		VizToggle.Text = "Barrier 표시 ON"
		VizToggle.BackgroundColor3 = Color3.fromRGB(115, 95, 45)
	else
		VizToggle.Text = "Barrier 표시 OFF"
		VizToggle.BackgroundColor3 = Color3.fromRGB(58, 58, 66)
	end
