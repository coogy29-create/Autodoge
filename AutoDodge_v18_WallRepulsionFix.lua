...
local function evaluateSpacePath(root, humanoid, direction, threatCount)
	...
	if direction.Magnitude < 0.01 then
		...
		if current.Nearest < WALL_STICK_CLEARANCE then
			local closeness =
				1 - math.clamp(current.Nearest / WALL_STICK_CLEARANCE, 0, 1)
			result.Risk += closeness * closeness * WALL_STICK_PENALTY
		end
		return result
	end
	...
	-- Directly determine whether this candidate moves toward the nearest barrier.
	if nearestBarrier and nearestBarrier.Distance < WALL_STICK_CLEARANCE then
		-- RayDirection points from player toward wall.
		...
		if towardDot > 0 then
			...
			local closeness =
				1 - math.clamp(
					nearestBarrier.Distance / WALL_STICK_CLEARANCE,
					0,
					1
				)
			...
			-- Strong reward for moving away from a wall we are already close to.
	...
	for _, futureTime in ipairs(SPACE_PATH_TIMES) do
		...
		if blocked then
			...
			result.Fatal = true
			result.Risk += SPACE_PATH_BLOCK_PENALTY
			break
		end
		...
		result.MinClearance =
			math.min(result.MinClearance, field.Nearest)

		result.WorstCloseRays =
			math.max(result.WorstCloseRays, field.CloseRays)

		-- This is the important wall-sticking guard:
		-- do not merely penalize collisions; penalize remaining close to a wall.
		if field.Nearest < WALL_STICK_CLEARANCE then
			local closeness =
				1 - math.clamp(
					field.Nearest / WALL_STICK_CLEARANCE,
					0,
					1
				)
		...
		if field.Nearest < WALL_STICK_HARD_CLEARANCE then
			local hardCloseness =
				1 - math.clamp(
					field.Nearest / WALL_STICK_HARD_CLEARANCE,
					0,
					1
				)
		...
		if field.Nearest < SPACE_HARD_CLEARANCE then
			result.Fatal = true
			result.Risk += SPACE_HARD_PENALTY
		end

		local localLoss =
			math.max(0, previousField.Average - field.Average)

		local totalLoss =
			math.max(0, current.Average - field.Average)
		...
		local escapeGain =
			math.max(0, field.Average - current.Average)

		result.EscapeGain =
			math.max(result.EscapeGain, escapeGain)
		...
		if data.Ready and data.Part and data.Part.Parent then
			...
			if speed >= MIN_PROJECTILE_SPEED then
				...
				if currentDistance <= MAX_TRACK_DISTANCE then
					...
					if (not knownOnly) or profile.Stable then
						local dangerRadius =
							PLAYER_RADIUS + projectileRadius(data.Part) + EXTRA_MARGIN
...
local function evaluateProjectileField(rootPosition, humanoid, direction, threats, steps)
	...
	for _, threat in ipairs(threats) do
		...
		for i = 0, steps do
			...
			local distance = (projectileFuture - playerFuture).Magnitude
...
local function chooseDirection(character, humanoid, root, knownOnly, learningKnownDodge)
	...
	local shouldIntervene =
		...
		and currentEval.NearestImpact <= TRIGGER_TIME
