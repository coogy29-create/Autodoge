...
local WALL_VIZ_RAYS = 18
local WALL_VIZ_DISTANCE = 18
local WALL_VIZ_UPDATE_INTERVAL = 0.15
...
local function getWallVizPart(rayIndex, bandIndex)
	...
	part.Size = Vector3.new(0.16, 0.16, 1)
	part.Transparency = 1
	...
	for rayIndex = 1, WALL_VIZ_RAYS do
		...
		for bandIndex, band in ipairs(WallVizBands) do
			...
			if result then
				...
				local middleDistance =
					(band.From + band.To) * 0.5
...
local function solidWallRaycast(origin, direction)
