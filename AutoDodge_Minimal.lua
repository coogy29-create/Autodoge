...
local SAMPLE_STEP = 0.035
local DECISION_INTERVAL = 0.03

local TRIGGER_TIME = 0.42
local HARD_TRIGGER_TIME = 0.22

local PLAYER_RADIUS = 2.6
local EXTRA_MARGIN = 1.15
local SAFE_EXTRA = 0.75

local MIN_PROJECTILE_SPEED = 3
local MAX_TRACK_DISTANCE = 120

local VELOCITY_SMOOTH = 0.5
local ACCELERATION_SMOOTH = 0.1
local MAX_ACCELERATION = 3500

local WALL_CHECK_DISTANCE = 5.5
local FLOOR_CHECK_DISTANCE = 7

local MIN_DODGE_HOLD = 0.055
local MAX_DODGE_HOLD = 0.14

local MIN_REQUIRED_IMPROVEMENT = 0.12
local MOVEMENT_PENALTY = 0.22
local DIRECTION_CHANGE_PENALTY = 0.16
...
local function environmentRisk(character, root, direction)
	...
	if wall then
		...
		risk += 20 + (1 - math.clamp(dist / WALL_CHECK_DISTANCE, 0, 1)) * 30
	end

	local future = root.Position + dir * 4
...
local function projectileThreat(rootPosition, playerVelocity, data)
	...
	local projectileVelocity = data.Velocity

	if projectileVelocity.Magnitude < MIN_PROJECTILE_SPEED then
		return 0, math.huge, math.huge
	end
...
local function totalRisk(character, humanoid, root, direction)
	...
	for _, data in pairs(ProjectileData) do
		local projectileRisk, impactTime = projectileThreat(
			root.Position,
			playerVelocity,
			data
		)
		...
		if projectileRisk > 0 and impactTime < nearestImpact then
			nearestImpact = impactTime
		end
	end

	risk += environmentRisk(character, root, direction)

	return risk, nearestImpact
end
...
for i = 0, 15 do
	...
	CandidateDirections[#CandidateDirections + 1] =
		Vector3.new(
			math.cos(angle),
			0,
			math.sin(angle)
...
local function movementCost(direction, currentDirection)
	if direction.Magnitude < 0.01 then
		return 0
	end

	local cost = MOVEMENT_PENALTY

	if currentDirection.Magnitude > 0.01 then
		local dot = math.clamp(
			currentDirection.Unit:Dot(direction.Unit),
			-1,
			1
		)

		cost += (1 - dot) * DIRECTION_CHANGE_PENALTY
	end
...
local function chooseDirection(character, humanoid, root)
	...
	local currentRisk, currentImpact =
		totalRisk(
			character,
			humanoid,
			root,
	...
	if stopRisk < bestScore then
		bestScore = stopRisk
		bestRisk = stopRisk
		bestDirection = Vector3.zero
	end

	for _, direction in ipairs(CandidateDirections) do
		local risk =
			totalRisk(
				character,
				humanoid,
	...
	if not bestDirection then
		return nil, currentRisk, currentImpact
	end
	...
	return bestDirection, currentRisk, currentImpact
end
