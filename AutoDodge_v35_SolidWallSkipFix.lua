...
local WALL_VIZ_RAYS = 32
local WALL_VIZ_DISTANCE = 18
...
local WallVizFolder =
	AutoDodgeDataFolder:FindFirstChild("SolidWallVisualization")

if not WallVizFolder then
	WallVizFolder = Instance.new("Folder")
	WallVizFolder.Name = "SolidWallVisualization"
	WallVizFolder.Parent = AutoDodgeDataFolder
end

local WallVizSegments = {}
...
local WallVizBands = {
	{
		From = 0,
		To = BARRIER_HARD_MIN_DISTANCE,
		Color = Color3.fromRGB(255, 65, 65),
		Transparency = 0.34
	},
	{
		From = BARRIER_HARD_MIN_DISTANCE,
		To = BARRIER_STRONG_MIN_DISTANCE,
		Color = Color3.fromRGB(255, 125, 55),
		Transparency = 0.46
	},
	{
		From = BARRIER_STRONG_MIN_DISTANCE,
		To = BARRIER_COMFORT_DISTANCE,
		Color = Color3.fromRGB(255, 195, 65),
		Transparency = 0.58
	},
	{
		From = BARRIER_COMFORT_DISTANCE,
		To = BARRIER_PROACTIVE_DISTANCE,
		Color = Color3.fromRGB(255, 235, 110),
		Transparency = 0.70
	}
}

local function getWallVizPart(rayIndex, bandIndex)
	WallVizSegments[rayIndex] =
		WallVizSegments[rayIndex] or {}

	local existing =
		WallVizSegments[rayIndex][bandIndex]
	...
	part.Size = Vector3.new(0.85, 0.10, 1)
	part.Transparency = 1
	part.Parent = WallVizFolder

	WallVizSegments[rayIndex][bandIndex] = part

	return part
end

local function hideWallVisualization()
	for _, rayParts in pairs(WallVizSegments) do
		for _, part in pairs(rayParts) do
...
local function updateWallVisualization(force)
	...
	if not force
		and now - WallVizLastUpdate
			< WALL_VIZ_UPDATE_INTERVAL
	then
		return
	end
	...
	if not character or not root or not solidWallRaycast then
		LastWallVizHits = 0
		hideWallVisualization()
		return
	end
	...
	for rayIndex = 1, WALL_VIZ_RAYS do
		local angle =
			((rayIndex - 1) / WALL_VIZ_RAYS)
			* math.pi
			* 2
		...
		if result then
			...
			local hitDistance =
				(result.Position - rayOrigin).Magnitude

			for bandIndex, band in ipairs(WallVizBands) do
				local part =
					getWallVizPart(
						rayIndex,
						bandIndex
				...
				if segmentLength > 0.05 then
					local middleFromPlayer =
						(startFromPlayer + endFromPlayer)
						* 0.5
			...
			for bandIndex = 1, #WallVizBands do
				local part =
					getWallVizPart(
						rayIndex,
						bandIndex
...
local SolidWallCharacter = nil
local SolidWallRoot = nil
local SolidWallEnemyProj = nil
local SolidWallCollisionGroup = nil
local SolidWallBaseExclude = {}

local function rebuildSolidWallBaseExclude()
	local character, _, root = getCharacter()

	if not character or not root then
		SolidWallCharacter = nil
		SolidWallRoot = nil
		SolidWallEnemyProj = nil
		SolidWallCollisionGroup = nil
		SolidWallBaseExclude = {}
		return nil, nil
	end

	local collisionGroup = root.CollisionGroup

	if SolidWallCharacter == character
		and SolidWallRoot == root
		and SolidWallEnemyProj == EnemyProj
		and SolidWallCollisionGroup == collisionGroup
	then
		return SolidWallBaseExclude, collisionGroup
	end

	local exclude = {character}

	if EnemyProj then
		exclude[#exclude + 1] = EnemyProj
	end

	if AutoDodgeDataFolder then
		exclude[#exclude + 1] = AutoDodgeDataFolder
	end

	SolidWallCharacter = character
	SolidWallRoot = root
	SolidWallEnemyProj = EnemyProj
	SolidWallCollisionGroup = collisionGroup
	SolidWallBaseExclude = exclude

	return exclude, collisionGroup
end

local function canTreatAsSolidWall(result)
	...
	if instance == workspace.Terrain then
		-- Terrain steep/vertical faces count as walls.
		return math.abs(result.Normal.Y) < 0.82
	end
	...
	-- Real movement-blocking surfaces.
	if not instance.CanCollide then
		return false
	end

	-- Ignore mostly-horizontal surfaces as floor/ceiling.
	if math.abs(result.Normal.Y) >= 0.82 then
		return false
	end
...
local function makeWallRayParams(extraExclude, collisionGroup)
	local exclude = {}

	for _, obj in ipairs(SolidWallBaseExclude) do
		exclude[#exclude + 1] = obj
	end

	if extraExclude then
		for obj in pairs(extraExclude) do
...
solidWallRaycast = function(origin, direction)
	local baseExclude, collisionGroup =
		rebuildSolidWallBaseExclude()

	if not baseExclude then
		return nil
	end
	...
	local remaining = totalDistance
	local currentOrigin = origin
	local skipped = {}

	for _ = 1, SOLID_WALL_RAY_MAX_SKIPS do
		...
		if canTreatAsSolidWall(result) then
			-- Preserve total distance from the original origin so callers
			-- can keep using result.Position normally.
			return result
		end

		local hit = result.Instance

		-- Terrain cannot be individually excluded. If terrain is a floor-like
		-- hit, advance past it a tiny amount and continue.
		if hit == workspace.Terrain then
			local travelled =
				(result.Position - currentOrigin).Magnitude
				+ SOLID_WALL_RAY_EPSILON
...
local function getSpaceField(position)
	...
	for i = 0, SPACE_RAYS - 1 do
		...
		local distance =
			result
			and (result.Position - origin).Magnitude
...
local function getNearestBarrierInfo(position)
	...
	for i = 0, SPACE_RAYS - 1 do
		...
		local result =
			solidWallRaycast(
				origin,
				dir * BARRIER_PROACTIVE_DISTANCE
			)
...
WallVizToggle.Activated:Connect(function()
	...
	if SHOW_WALL_RANGES then
		...
		WallVizToggle.Text = "벽 범위 표시 OFF"
...
RunService.RenderStepped:Connect(function()
	...
	Info.Text =
		...
		.. "\nWall : "
		.. tostring(LastWallVizHits)
		.. "/"
		.. tostring(WALL_VIZ_RAYS)
		.. " / Risk : "
