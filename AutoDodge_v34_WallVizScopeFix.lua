...
local function getWallVizPart(rayIndex, bandIndex)
	...
	part.Size = Vector3.new(0.48, 0.08, 1)
	part.Transparency = 1
	...
	for rayIndex = 1, WALL_VIZ_RAYS do
		...
		if result then
			...
			for bandIndex, band in ipairs(WallVizBands) do
				...
				-- A point t studs from the player is
				-- (hitDistance - t) studs from the wall.
...
local function isSolidWallHit(result)
	...
	if instance == workspace.Terrain then
		return math.abs(result.Normal.Y) < 0.78
	end
	...
	-- Horizontal sensor rays should treat mostly-vertical faces as walls,
	-- not floors/ceilings.
	if math.abs(result.Normal.Y) >= 0.78 then
		return false
	end
...
solidWallRaycast = function(origin, direction)
	local params = rebuildSolidWallParams()
	if not params then
		return nil
	end
