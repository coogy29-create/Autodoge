...
local CANDIDATE_COUNT = 72
local REFINE_ANGLE = 2.5
...
local LEARNING_PATH_PREDICT_TIME = 1.50
local LEARNING_PATH_STEP = 0.05
...
Main.Size = UDim2.fromOffset(235, 210)
...
ModeLabel.Position = UDim2.fromOffset(10, 184)
...
local BarrierParts = {}
local BarrierSet = {}
local BarrierRayParams = nil

local function isBarrierPart(obj)
	return obj:IsA("BasePart")
		and string.lower(obj.Name) == "barrier"
end

local function rebuildBarrierList()
	local list = {}
	...
	if #BarrierParts == 0 then
		BarrierRayParams = nil
		return
	end

	local params = RaycastParams.new()
	params.FilterType = Enum.RaycastFilterType.Include
	params.FilterDescendantsInstances = BarrierParts
	params.IgnoreWater = true
	BarrierRayParams = params
end

local function scanAllBarriers()
	table.clear(BarrierSet)

	for _, obj in ipairs(workspace:GetDescendants()) do
		if isBarrierPart(obj) then
			BarrierSet[obj] = true
		end
	end
...
local function getBarrierFolder()
	return #BarrierParts > 0 and BarrierParts or nil
end

local function getBarrierParams()
	return BarrierRayParams
end

local function getSpaceField(position)
	local params = getBarrierParams()
	if not params then
		return {
			Nearest = math.huge,
			Average = SPACE_CHECK_DISTANCE,
			CloseRays = 0,
			Corner = false
		}
	end
	...
	for i = 0, SPACE_RAYS - 1 do
		...
		local result = workspace:Raycast(
			origin,
			dir * SPACE_CHECK_DISTANCE,
			params
		)

		local distance = SPACE_CHECK_DISTANCE
	...
	return {
		...
		CloseRays = closeRays,
		Corner = closeRays >= SPACE_CORNER_CLOSE_RAYS
	}
end
...
local function barrierPathBlocked(fromPosition, toPosition)
	...
	for _, offset in ipairs(offsets) do
		local origin = fromPosition + Vector3.new(0, 1, 0) + offset
...
local function evaluateSpacePath(root, humanoid, direction, threatCount)
	...
	local currentBarrierDistance =
		nearestBarrier and nearestBarrier.Distance or math.huge
	...
	if nearestBarrier and currentBarrierDistance < BARRIER_COMFORT_DISTANCE then
		local towardDot = unit:Dot(nearestBarrier.RayDirection)

		if towardDot > 0 then
			result.MovingTowardWall = true
			local closeness =
				1 - math.clamp(currentBarrierDistance / BARRIER_COMFORT_DISTANCE, 0, 1)
	...
	for _, futureTime in ipairs(SPACE_PATH_TIMES) do
		...
		result.FutureBarrierDistance = futureBarrierDistance
		result.MinClearance = math.min(result.MinClearance, field.Nearest)
		result.WorstCloseRays = math.max(result.WorstCloseRays, field.CloseRays)
		...
		if currentBarrierDistance < BARRIER_COMFORT_DISTANCE
			and futureBarrierDistance + BARRIER_DISTANCE_EPSILON < currentBarrierDistance
		then
			local loss = currentBarrierDistance - futureBarrierDistance
			result.BarrierDistanceLoss = math.max(result.BarrierDistanceLoss, loss)
			result.Risk +=
				BARRIER_HARD_REJECT_SCORE
				+ loss * BARRIER_DISTANCE_LOSS_WEIGHT
		end

		if futureBarrierDistance < BARRIER_STRONG_MIN_DISTANCE then
			local closeness =
				1 - math.clamp(futureBarrierDistance / BARRIER_STRONG_MIN_DISTANCE, 0, 1)
		...
		elseif futureBarrierDistance < BARRIER_COMFORT_DISTANCE then
			local closeness =
				1 - math.clamp(
					(futureBarrierDistance - BARRIER_STRONG_MIN_DISTANCE)
						/ math.max(BARRIER_COMFORT_DISTANCE - BARRIER_STRONG_MIN_DISTANCE, 0.001),
					0,
					1
				)
		...
		then
			local gain = futureBarrierDistance - currentBarrierDistance
			result.Risk -=
				gain
				* BARRIER_DISTANCE_GAIN_BONUS
				* denseMultiplier
		end

		local localLoss = math.max(0, previousField.Average - field.Average)
		local totalLoss = math.max(0, current.Average - field.Average)

		result.Risk +=
			(
				localLoss * localLoss * SPACE_LOSS_WEIGHT
				+ totalLoss * totalLoss * SPACE_LOSS_WEIGHT * 0.6
			)
			* denseMultiplier
		...
		if field.Corner then
			result.Risk +=
				SPACE_CORNER_WEIGHT
				* (1 + field.CloseRays / SPACE_RAYS)
				* denseMultiplier
		end

		local escapeGain = math.max(0, field.Average - current.Average)
		result.EscapeGain = math.max(result.EscapeGain, escapeGain)
...
local function barrierDistance(position, direction)
	...
	local result = workspace:Raycast(
		origin,
		direction.Unit * REAR_BARRIER_CHECK_DISTANCE,
		params
	)
...
local function wallProximityAt(position, params)
	...
	for i = 0, WALL_RADIAL_RAYS - 1 do
		...
		local result = workspace:Raycast(position, direction * WALL_CHECK_DISTANCE, params)
		...
		if result then
			...
			if distance < WALL_COMFORT_DISTANCE then
				closeCount += 1
				local closeness = 1 - math.clamp(distance / WALL_COMFORT_DISTANCE, 0, 1)
				totalPenalty += closeness * closeness * WALL_PROXIMITY_WEIGHT
			end

			if distance < WALL_HARD_DISTANCE then
				local hardCloseness = 1 - math.clamp(distance / WALL_HARD_DISTANCE, 0, 1)
				totalPenalty += 60 + hardCloseness * 80
			end
		end
	end

	if closeCount >= 2 then
		totalPenalty += (closeCount - 1) * WALL_CORNER_WEIGHT
	end
...
local function environmentRisk(character, root, direction)
	...
	local currentPosition = root.Position + Vector3.new(0, 1, 0)
	local risk = 0

	local currentWallRisk = wallProximityAt(currentPosition, params)
	risk += currentWallRisk * 0.35

	if direction.Magnitude > 0.01 then
		local dir = direction.Unit
		local forwardWall = workspace:Raycast(
			currentPosition,
		...
		if forwardWall then
			local distance = (forwardWall.Position - currentPosition).Magnitude
			if distance < WALL_COMFORT_DISTANCE then
				local approach = 1 - math.clamp(distance / WALL_COMFORT_DISTANCE, 0, 1)
				risk += approach * approach * WALL_APPROACH_WEIGHT
			end
			if distance < WALL_HARD_DISTANCE then
				risk += 90
			end
		...
		local futurePosition = currentPosition + dir * WALL_FUTURE_DISTANCE
		local futureWallRisk = wallProximityAt(futurePosition, params)
		risk += futureWallRisk

		local floor = workspace:Raycast(
			futurePosition + Vector3.new(0, 1, 0),
		...
		local floor = workspace:Raycast(
			currentPosition + Vector3.new(0, 1, 0),
			Vector3.new(0, -FLOOR_CHECK_DISTANCE, 0),
			params
		)
...
local function buildThreatSnapshot(root, knownOnly)
	...
	for _, data in pairs(ProjectileData) do
		if data.Ready and data.Part and data.Part.Parent then
			if (data.Part.Position - root.Position).Magnitude <= MAX_TRACK_DISTANCE then
				local profile = getDamageProfile(data.Signature)
	...
	local predictTime =
		PREDICT_TIME
		+ (
			nearbyCount >= DENSE_THREAT_COUNT
			and DENSE_LOOKAHEAD_BONUS
			or 0
		)

	local steps = math.floor(predictTime / SAMPLE_STEP + 0.5)

	for _, data in pairs(ProjectileData) do
		if data.Ready and data.Part and data.Part.Parent then
			local speed = data.Velocity.Magnitude
			if speed >= MIN_PROJECTILE_SPEED then
				local currentDistance = (data.Part.Position - root.Position).Magnitude
				if currentDistance <= MAX_TRACK_DISTANCE then
					...
					if (not knownOnly) or profile.Stable then
						local projRadius =
							data.Radius
							or projectileRadius(data.Part, data.Object)
						...
						if projRadius >= LARGE_PROJECTILE_RADIUS then
							largeExtra =
								math.min(
									...
									projRadius * LARGE_PROJECTILE_EXTRA_SCALE
								)
						end
						...
						local threat = {
							Signature = data.Signature,
							Damage = profile.BaseDamage and (profile.BaseDamage * getDifficulty()) or nil,
							BaseDamage = profile.BaseDamage,
							DamageStable = profile.Stable,
							DamageConfidence = profile.BaseConfidence or profile.Confidence,
						...
						for i = 0, steps do
							...
							threat.Positions[i + 1] = predictProjectile(data, t)
						end
...
local function closestRelativeSegment(r0, r1)
	...
	return (r0 + d * alpha).Magnitude, alpha
end
...
local function betterCandidate(a, b)
	...
	local aLosesBarrierDistance =
		(a.Eval.SpaceBarrierDistanceLoss or 0) > BARRIER_DISTANCE_EPSILON

	local bLosesBarrierDistance =
		(b.Eval.SpaceBarrierDistanceLoss or 0) > BARRIER_DISTANCE_EPSILON
	...
	local aSevereTrap =
		(a.Eval.SpaceCloseRays or 0) >= SPACE_CORNER_CLOSE_RAYS
		and (a.Eval.SpaceMinClearance or math.huge) < SPACE_COMFORT_DISTANCE

	local bSevereTrap =
		(b.Eval.SpaceCloseRays or 0) >= SPACE_CORNER_CLOSE_RAYS
		and (b.Eval.SpaceMinClearance or math.huge) < SPACE_COMFORT_DISTANCE
	...
	if math.abs(a.Eval.MinimumClearance - b.Eval.MinimumClearance) > 0.05 then
		return a.Eval.MinimumClearance > b.Eval.MinimumClearance
	end

	return a.Direction.Magnitude < b.Direction.Magnitude
end
		...
		if LEARNING_MODE then
			...
			if healthRatio < LEARNING_MIN_HEALTH_RATIO then
				local emergencyDirection, risk, impactTime =
					chooseDirection(character, humanoid, root, false, false)
			...
			local knownDirection, knownRisk, knownImpact =
				chooseDirection(character, humanoid, root, true, true)
