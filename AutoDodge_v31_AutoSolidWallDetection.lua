...
local function getSolidWallParams()
	local exclude = {}

	local character = LocalPlayer.Character
	if character then
		exclude[#exclude + 1] = character
	end
...
local function isSolidWallHit(result)
	...
	if instance:IsA("BasePart") then
		...
		-- Horizontal wall rays should primarily hit vertical faces.
		-- This rejects most floor/ceiling/flat-platform hits.
		if math.abs(result.Normal.Y) > 0.72 then
			return false
		end
	...
	-- Terrain can also be a real impassable boundary.
	if instance == workspace.Terrain then
		return math.abs(result.Normal.Y) <= 0.72
	end
...
local function solidWallRaycast(origin, direction)
	local params = getSolidWallParams()
...
local function getNearestBarrierInfo(position)
	...
	for i = 0, WALL_RADIAL_RAYS - 1 do
		...
		local result =
			solidWallRaycast(
				origin,
				direction * BARRIER_PROACTIVE_DISTANCE
			)
...
local function wallProximityAt(position, params)
	...
	return nearest, closeCount, totalPenalty
end
...
local function environmentRisk(character, root, direction)
	...
	local result =
		solidWallRaycast(
			origin,
			direction.Unit * WALL_CHECK_DISTANCE
		)
