...
local function evaluateSpacePath(root, humanoid, direction, threatCount)
		...
		then
			...
			local bonus =
				...
				and (
					...
					+ BARRIER_DISTANCE_GAIN_EXTRA_WEIGHT
				)
				or (
					BARRIER_PROACTIVE_ESCAPE_BONUS
					+ BARRIER_DISTANCE_GAIN_EXTRA_WEIGHT
			...
			-- Moving away from the real game Barrier directly lowers risk.
			-- The farther the candidate gains distance, the larger the reward.
			result.Risk -=
				gain
				* bonus
				* denseMultiplier
	...
	if currentBarrierDistance < math.huge
		and result.FutureBarrierDistance < math.huge
	then
		if result.FutureBarrierDistance
			< currentBarrierDistance - BARRIER_DISTANCE_EPSILON
		then
		...
		elseif result.FutureBarrierDistance
			> currentBarrierDistance + BARRIER_DISTANCE_EPSILON
		then
			result.BarrierRiskMultiplier =
...
local function evaluateCandidate(
	...
	field.BarrierRiskMultiplier = space.BarrierRiskMultiplier or 1.0
...
local function betterCandidate(a, b)
	...
	local aBarrierFuture =
		a.Eval.SpaceFutureBarrierDistance
		or math.huge

	local bBarrierFuture =
		b.Eval.SpaceFutureBarrierDistance
		or math.huge

	if math.min(aBarrierFuture, bBarrierFuture)
		< BARRIER_COMFORT_DISTANCE
		and math.abs(aBarrierFuture - bBarrierFuture) > 0.20
	then
		return aBarrierFuture > bBarrierFuture
	end
	...
	local aFutureClearance =
		a.Eval.SpaceFutureBarrierDistance
		or math.huge

	local bFutureClearance =
		b.Eval.SpaceFutureBarrierDistance
		or math.huge

	if a.Eval.ImmediateHits == 0
		and b.Eval.ImmediateHits == 0
		and a.Eval.HardHits == 0
		and b.Eval.HardHits == 0
		and aFutureClearance >= LATERAL_TIE_BREAK_CLEARANCE
		and bFutureClearance >= LATERAL_TIE_BREAK_CLEARANCE
	then
		local aRetreat =
			math.max(
				0,
				-(a.Eval.ReferenceDot or 0)
			)

		local bRetreat =
			math.max(
				0,
				-(b.Eval.ReferenceDot or 0)
			)

		if math.abs(aRetreat - bRetreat) > 0.08 then
			return aRetreat < bRetreat
		end

		local aLateral = a.Eval.LateralAmount or 0
		local bLateral = b.Eval.LateralAmount or 0

		if math.max(aLateral, bLateral) >= LATERAL_TIE_BREAK_DOT
			and math.abs(aLateral - bLateral) > 0.08
		then
...
local function chooseDirection(character, humanoid, root, knownOnly, learningKnownDodge)
	local liveUserDirection = flatVector(humanoid.MoveDirection)

	if liveUserDirection.Magnitude > 1 then
		liveUserDirection = liveUserDirection.Unit
	end

	local liveBaseDirection = getBaseDirection(humanoid, root)

	-- Do not let AutoDodge's own Humanoid:Move() direction become the
	-- reference on the next decision. That feedback loop could turn one
	-- necessary retreat into repeated backwards movement.
	if not DODGING then
		if liveUserDirection.Magnitude > 0.01 then
			StableReferenceDirection = liveUserDirection.Unit
		elseif liveBaseDirection.Magnitude > 0.01 then
			StableReferenceDirection = liveBaseDirection.Unit
		end
	end
	...
	local baseDirection = referenceDirection
...
RunService:BindToRenderStep(
	...
	function()
		...
		if not character then
			DODGING = false
			StableReferenceDirection = Vector3.zero
			enableControls()
			return
		end
