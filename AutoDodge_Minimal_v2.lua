...
local MAX_DODGE_HOLD = 0.085
local DODGE_COOLDOWN = 0.035
...
local function totalRisk(character, humanoid, root, direction)
	...
	for _, data in pairs(ProjectileData) do
		local risk, impactTime = projectileThreat(
			root.Position,
			playerVelocity,
			data
		)
	...
	return projectileScore + envScore, nearestImpact, worstRisk
end
...
RunService:BindToRenderStep(
	...
	function()
		...
		if now - lastDecision >= DECISION_INTERVAL then
			...
			if direction and now >= nextDodgeAllowed then
