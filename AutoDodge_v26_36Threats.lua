...
local PERF_MAX_EVALUATED_THREATS = 36
local PERF_DENSE_SKIP_REFINE = 12
local PERF_APPROACH_DOT_MIN = -0.20
local PERF_FAR_DISTANCE = 55
local PERF_FAR_APPROACH_DOT = 0.10
local PERF_MIN_TTC_KEEP = 0.85
...
local function buildThreatSnapshot(root, knownOnly)
	...
	for _, data in pairs(ProjectileData) do
		if data.Ready and data.Part and data.Part.Parent then
			...
			if speed >= MIN_PROJECTILE_SPEED then
				local offset = data.Part.Position - root.Position
				...
				if distance <= MAX_TRACK_DISTANCE then
					...
					if (not knownOnly) or profile.Stable then
						...
						local keep =
							distance <= PERF_FAR_DISTANCE
							or approachDot >= PERF_FAR_APPROACH_DOT

						if keep then
							local closingSpeed =
								math.max(
									1,
									speed * math.max(approachDot, 0.05)
								)

							local ttc = distance / closingSpeed

							if approachDot >= PERF_APPROACH_DOT_MIN
								or ttc <= PERF_MIN_TTC_KEEP
							then
								candidates[#candidates + 1] = {
									Data = data,
									Distance = distance,
									TTC = ttc,
									Approach = approachDot
								}
							end
						end
					end
	...
	table.sort(candidates, function(a, b)
		local aScore =
			a.TTC
			+ a.Distance * 0.004
			- math.max(a.Approach, 0) * 0.20

		local bScore =
			b.TTC
			+ b.Distance * 0.004
			- math.max(b.Approach, 0) * 0.20
	...
	for index = 1, limit do
		...
		local threat = {
			...
			Damage =
				profile.BaseDamage
				and (profile.BaseDamage * getDifficulty())
				or nil,
